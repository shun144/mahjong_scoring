 ...
 */
export function detectStandardYaku(interp: StandardInterpretation, ctx: YakuContext): YakuResult[] {
  ...
  if (
    allTriplets &&
    allTypes.every(isTerminalOrHonor) &&
    allTypes.some(isHonorType) &&
    allTypes.some((t) => !isHonorType(t))
  ) {
  ...
  // --- 3翻 ---
  // 混一色 / 清一色
  const suitsUsed = new Set(allTypes.filter((t) => !isHonorType(t)).map((t) => Math.floor(t / 9)));
