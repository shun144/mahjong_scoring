 ...
 * 牌種インデックス(0-33)の枚数配列を入力とし、あり得る全ての分解を列挙する
...
export function decomposeChiitoitsu(counts: readonly number[]): ChiitoitsuDecomposition | null {
  const pairs: number[] = [];
  for (let i = 0; i < 34; i++) {
    const c = counts[i];
    if (c === 0) continue;
...
/** 国士無双として成立するか判定する。 */
export function decomposeKokushi(counts: readonly number[]): KokushiDecomposition | null {
