import type { StandardInterpretation } from "./interpretation";
import type { YakuResult } from "./score";
import { isGreenType, isHonorType, isTerminal } from "./tileType";
import { allMemberTypes } from "./yaku";
...
export function detectYakuman(
  ...
  if (
    interp.sets.every((s) => s.kind === "triplet") &&
    allTypes.every(isTerminal)
  ) {
  ...
  const windTripletCount = interp.sets.filter(
    (s) => s.kind === "triplet" && WIND_TYPES.includes(s.tileType),
  ).length;
  if (windTripletCount === 4) {
    results.push({ name: "大四喜", han: 13 });
  } else if (windTripletCount === 3 && WIND_TYPES.includes(interp.pair.tileType)) {
